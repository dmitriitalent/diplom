...
if (!isCommandant && !isAuthor) {
	await navigateTo("/orders/" + id);
}
if (original.value.status === "closed") {
	await navigateTo("/orders/" + id);
}

const form = ref<OrderDtoCreate>({
...
const saveOrder = async () => {
	...
	try {
		...
		await $fetch("/api/order/edit?id=" + id, {
			method: "PUT",
			body: form.value,
		});
			...
			<div :class="$style.form">
				...
				<div :class="$style.right">
					<div :class="$style.field">
						<h3 :class="$style.label">Тип заявки</h3>
						<UiSelect v-model="form.type" :options="orderTypeOptions" />
					</div>
