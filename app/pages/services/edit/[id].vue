...
const { data: original } = await useAsyncData<ServiceDtoById>(
	"service-edit-" + id,
	() => $fetch<ServiceDtoById>("/api/service/byId?id=" + id),
);

if (!original.value) {
	throw createError({ statusCode: 404, message: "Услуга не найдена" });
}

const isOwner = original.value.ownerId === self?.id;
if (!isAdmin && !isOwner) {
	await navigateTo("/services/" + id);
}
...
const removeExistingImage = (guid: string) => {
	existingImages.value = existingImages.value.filter((img) => img.fileGuid !== guid);
	previewChange.value++;
};
...
const saveService = async () => {
	...
	try {
		...
		await $fetch("/api/service/edit?id=" + id, {
			method: "PATCH",
			body: form.value,
		});
					...
					<div :class="$style.field">
						...
						<div :class="$style.statusRow">
							<UiButton
								...
								:class="[
									$style.statusBtn,
									form.status === opt.value && $style.statusBtnActive,
								]"
								inset
