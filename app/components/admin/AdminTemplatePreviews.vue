...
const previewUrl = (key: string) => `/api/staticfiles/${key}?v=${bumpVersion.value}`;
...
<template>
	<div :class="$style.wrap">
		<div :class="$style.header">
			...
			<p :class="$style.hint">
				Изображения хранятся на BFF и отображаются в выборе шаблона при создании/редактировании материалов.
			</p>
		</div>
				...
				>
					<div :class="$style.preview">
						...
						<div v-else :class="$style.previewEmpty">
							<Icon name="mdi:image-off-outline" :class="$style.emptyIcon" />
					...
					<div :class="$style.meta">
						...
						<div :class="$style.actions">
							...
							>
								<Icon
									:name="uploadingKey === tpl.key ? 'mdi:loading' : 'mdi:upload'"
									:class="[$style.btnIcon, uploadingKey === tpl.key && $style.spinning]"
								/>
							...
							>
								<Icon
									:name="deletingKey === tpl.key ? 'mdi:loading' : 'mdi:trash-can-outline'"
									:class="[$style.btnIcon, deletingKey === tpl.key && $style.spinning]"
								/>
							</button>
						</div>
						...
						<input
							...
							accept="image/*"
							hidden
							:ref="(el) => { fileInputs[tpl.key] = el as HTMLInputElement | null }"
