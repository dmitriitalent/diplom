...
import { useVerifications, type Verification } from "~/composables/useVerifications";
...
<template>
	<div :class="$style.wrap">
		...
		<div v-if="loading && pending.length === 0" :class="$style.muted">Загрузка…</div>
			...
			>
				<div :class="$style.itemHead">
					<div :class="$style.meta">
						<RouterLink :to="`/profile/${v.userId}`" :class="$style.userLink">
							<Icon name="mdi:account-outline" :class="$style.userIcon" />
				...
				<div :class="$style.body">
					...
					<div :class="$style.controls">
						...
						<div :class="$style.actions">
							...
							>
								<Icon
									:name="busy[v.id] === 'approve' ? 'mdi:loading' : 'mdi:check-circle-outline'"
									:class="[$style.btnIcon, busy[v.id] === 'approve' && $style.spinning]"
								/>
								Одобрить
							</button>
							<button
							...
							>
								<Icon
									:name="busy[v.id] === 'reject' ? 'mdi:loading' : 'mdi:close-circle-outline'"
									:class="[$style.btnIcon, busy[v.id] === 'reject' && $style.spinning]"
								/>
								Отклонить
							</button>
						</div>
