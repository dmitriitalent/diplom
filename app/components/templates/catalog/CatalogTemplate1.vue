...
const statusBadge = computed(() => {
	const map: Record<string, { bg: string; color: string; label: string; icon: string }> = {
		active: { bg: "#efffde", color: "#1c5b1c", label: "В продаже", icon: "mdi:check-circle-outline" },
		reserved: { bg: "#fff3c8", color: "#7a5a00", label: "Забронирован", icon: "mdi:bookmark-outline" },
		sold: { bg: "rgba(38,28,7,0.08)", color: "rgba(38,28,7,0.65)", label: "Продан", icon: "mdi:archive-outline" },
	...
	<div :class="[$style.wrapper, ...deviceClassList]">
		...
		<div :class="$style.container">
			...
			<div :class="$style.crumbs">
				<RouterLink to="/catalog" :class="$style.crumbLink">Каталог</RouterLink>
			...
			<div :class="$style.grid">
				...
				<div :class="$style.left">
					...
					<article :class="$style.article">
						...
						<p :class="$style.articleText">{{ product.description }}</p>
					</article>

					<!-- Safe deal note -->
					<div :class="$style.safeDeal">
						<Icon name="mdi:shield-check-outline" :class="$style.safeDealIcon" />
						<div :class="$style.safeDealText">
							<span :class="$style.safeDealTitle">Сделка между соседями</span>
							<span :class="$style.safeDealDesc">
								Договаривайтесь о встрече в холле, на этаже или в комнате. Деньги передавайте лично — Hostelite не проводит платежи.
							</span>
						</div>
					</div>
				</div>
				...
				<aside :class="$style.sidebar">
					...
					<!-- Owner -->
					<div v-if="product.owner" :class="[$style.sideRow, $style.sideRowBorder]">
						<RouterLink :to="`/profile/${product.owner.id}`" :class="$style.authorLink">
							<img
