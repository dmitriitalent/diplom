	...
	>
		...
		<div :class="$style.body">
			<div :class="$style.top">
				...
				<span
					v-if="activity.moderationStatus && activity.moderationStatus !== 'approved'"
					:class="[$style.badge, $style['badge_' + activity.moderationStatus]]"
				>
					{{ statusLabel[activity.moderationStatus] ?? activity.moderationStatus }}
				</span>
			</div>
			...
			<div :class="$style.meta">
				<span v-if="activity.location" :class="$style.metaItem">
					<Icon name="mdi:map-marker-outline" :class="$style.metaIcon" />
					{{ activity.location }}
				</span>
				<span v-if="activity.startTime" :class="$style.metaItem">
					<Icon name="mdi:clock-outline" :class="$style.metaIcon" />
					{{ formatDate(activity.startTime) }}
					<span v-if="activity.endTime"> — {{ formatDate(activity.endTime) }}</span>
				</span>
				<span v-if="activity.participants?.length" :class="$style.metaItem">
					<Icon name="mdi:account-group-outline" :class="$style.metaIcon" />
